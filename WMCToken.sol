...
contract WMCToken is Context, IBEP20, Ownable {
  ...
  address private _previoustaxWallet = _taxWallet;

  //taxfee beetween 0 and 500 to get 2 decimals (5=0.05% // 100=1% // 500=5%)
  uint256 private _taxFee = 0;
  uint256 private _previoustaxFee = _taxFee;


  constructor() public {
    _name = "World Masonic Coin";
    _symbol = "WMC";
    _decimals = 8;
    ...
    _mint(msg.sender, 33000000000 * (10 ** 8));
  }
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(_isExcludedFromFee[sender] || _isExcludedFromFee[recipient] || _taxFee == 0){
      ...
      _balances[_taxWallet] = _balances[_taxWallet].add(fee);
      emit Transfer(sender, _taxWallet, fee);
    }
  ...
  function getPreviousTaxWallet() external view returns(address) {
      return _previoustaxWallet;
  }

  function setTaxWallet(address taxWallet) external onlyOwner() {
    require(taxWallet != address(0), "BEP20: fundwallet from the zero address");

      _previoustaxWallet = _taxWallet;
      _taxWallet = taxWallet;

      _isExcludedFromFee[_taxWallet] = true;
      _isExcludedFromFee[_previoustaxWallet] = false;
